...
<body>
	...
	<xsl:attribute name="class">
		<xsl:choose>
			<xsl:when test="$reading-mode = 'explore'">
	...
	<div id="utilities" role="controlset">
		<ul>
			<li><a href="#info" title="Help Info (i)" class="entypo-info-circled"><span>Help information (i key)</span></a></li>
			...
			<li>
				<button id="quiet" title="Quiet Mode (q)">
					<xsl:attribute name="class">
						...
						<xsl:if test="not($reading-mode = 'explore')">
			...
			<li>
				<button id="explore" title="Explore Mode (e)">
					<xsl:attribute name="class">
						...
						<xsl:if test="$reading-mode = 'explore'">
							<xsl:text> active</xsl:text>
						</xsl:if>
					</xsl:attribute>
			...
			<xsl:if test="//params/ds-current-chapter/item">
				<li>
					<button id="set-bookmark" class="entypo-tag" type="button">
						<xsl:if test="$bookmark = //params/ds-current-chapter/item">
						...
						<xsl:attribute name="title">
							<xsl:choose>
								<xsl:when test="$bookmark = //params/ds-current-chapter/item">
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<a class="entypo-bookmark">
								...
								<span>Go to chapter <xsl:value-of select="$bookmark"/> (g)</span>
							</a>
						</xsl:otherwise>
					</xsl:choose>
				</li>
	...
	<footer>
		...
		<div id="info" title="Keyboard Shortcuts">
			...
			<div id="settings-utilities">
				...
				<dl>				
					<dt>s</dt>
					<dt class="entypo-info-circled"><span>i Icon</span></dt>
					<dd>This Help Page</dd>
					<dt>d</dt>
		...
		<div id="docs">
			<ul>
				<li><a href="{$root}/api-docs">API Docs</a></li>
...
<script><xsl:if test="//params/ds-current-chapter/item">
	$("button#set-bookmark").click(function(){
		$.cookie('bookmark', '<xsl:value-of select="$ds-current-chapter"/>', {path:'/'});
	...
	chaptersTimeline("#chapters-timeline","/humphrey_clinker/back/chapter-json",360,2000);
</xsl:if>
<xsl:if test="//params/current-path = '/chapter'">
horizontalTimeline("#toc-visualized","/humphrey_clinker/back/chapter-json",960,300);
</xsl:if>
</script>
