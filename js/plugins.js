...
function horizontalTimeline(domElement, dataSet, width, height) {
	var	hmargin = 10,
		vmargin = 80,
		root,
		xml,
		format = d3.time.format("%Y-%m-%d"),
		readFormat = d3.time.format("%b. %e");

/*	The canvas */
	var visualTOC = d3.selectAll(domElement).append("svg:svg")
		.attr("viewBox","0 0 " + width + " " + height)
		.attr("preserveAspectRatio", "xMinYMin");


/* Chapter Scale */
	var chapterScale = d3.scale.linear()
		.domain([-1, 86])
		.range([hmargin, width - hmargin]),

		chapterAxis = d3.svg.axis()
			.scale(chapterScale)
			.orient("top")
			.tickValues([1,10,20,30,40,50,60,70,80])
		    .tickSize(8,0);

	visualTOC.append("svg:g")
		.attr("id","chapter-line")
		.attr("class","scale")
	...
	d3.selectAll("g#chapter-line g.tick.major line")
		.attr("x1","8");

/* Date Scale */
	var dateScale = d3.time.scale()
		.domain([new Date(1770, 2, 26), new Date(1770, 10, 26)])
		.range([hmargin, width - hmargin]),
	...
	visualTOC.append("svg:g")
		...
		.append("svg:text")
			...
			.attr("text-anchor","middle")
			.text("Date Sent");
	d3.selectAll("g#date-line g.tick.major line")
		.attr("x1","-8");

/* Data */
	d3.xml(dataSet, function(xml) {
		visualTOC.append("g").attr("class","vtoc-chapters");

	/* Plotting Chapters */
		visChapter = visualTOC.select(".vtoc-chapters").selectAll("g")
			.data(xml.documentElement.getElementsByTagName("chapter"));

			visChapter.enter().append("a")
				.attr("class", function(d) { return d.getElementsByTagName("slug")[0].textContent + " chapter"})
				.attr("chapter", function(d) { return d.getElementsByTagName("order")[0].textContent})
				.attr("xlink:href", function(d) {return d.getElementsByTagName("url")[0].textContent});
				
	/* Plotting Link */
			visChapter.append("line")
				.attr("y1", (vmargin))
			    .attr("x1", function(d) { return chapterScale(d.getElementsByTagName("order")[0].textContent) })
			    .attr("y2", function(d) {
			    	if (d.getElementsByTagName("date")[0].textContent == "Date Unknown") { return height / 2 }
			    	else { return (height - (vmargin)) }

			    })
			    .attr("x2", function(d) { 
			    	if (d.getElementsByTagName("date")[0].textContent == "Date Unknown") { return chapterScale(d.getElementsByTagName("order")[0].textContent); }
			    	else { return dateScale(format.parse(d.getElementsByTagName("date")[0].textContent)) }
			    })
				.append("title")
					.text(function(d) {
					if (d.getElementsByTagName("date")[0].textContent == "Date Unknown") { return "Ch. "+d.getElementsByTagName("order")[0].textContent+". "+d.getElementsByTagName("title")[0].textContent+" (Date unknown.)" }
					else {
						var sentDate = new Date(d.getElementsByTagName("date")[0]),
							sentMonth = sentDate.getMonth(),
							sentDay = sentDate.getDay();
						return "["+readFormat(new Date(1770,sentMonth,sentDay))+"] "+d.getElementsByTagName("order")[0].textContent+". "+d.getElementsByTagName("title")[0].textContent+" (From "+d.getElementsByTagName("name")[0].textContent+".)" }});

		/* Chapter */
			visChapter.append("circle")
					.attr("r",6)
					.attr("cx", function(d) { return chapterScale(d.getElementsByTagName("order")[0].textContent) })
					.attr("cy", (vmargin))
				.append("title")
					.text(function(d) {
					if (d.getElementsByTagName("date")[0].textContent == "Date Unknown") { return "Ch. "+d.getElementsByTagName("order")[0].textContent+". "+d.getElementsByTagName("title")[0].textContent+" (Date unknown.)" }
				...
				.append("title")
					...
					if (d.getElementsByTagName("date")[0].textContent == "Date Unknown") { return "Ch. "+d.getElementsByTagName("order")[0].textContent+". "+d.getElementsByTagName("title")[0].textContent+" (Date unknown.)" }
						...
						var sentDate = new Date(d.getElementsByTagName("date")[0]),
							...
							sentDay = sentDate.getDay();
						return "["+readFormat(new Date(1770,sentMonth,sentDay))+"] "+d.getElementsByTagName("order")[0].textContent+". "+d.getElementsByTagName("title")[0].textContent+" (From "+d.getElementsByTagName("name")[0].textContent+".)" }});
		$(domElement+" a.chapter *").tipsy({gravity:'s'});
		$(domElement+" a.chapter *").tipsy({trigger:'focus',gravity:'s'});
	});
}

function chaptersTimeline(domElement, dataSet, width, height) {
	var	rmargin = 100,
		lmargin = 30,
		vmargin = 20,
		root,
...
/* Dates */
	...
	visualTOC.append("svg:g")
		.attr("id","date-line")
		.attr("class","scale")
		.attr("transform","translate(" + (width - (rmargin)) + ",0)")
		.call(dateAxis)
		.append("svg:text")
			.attr("y","32")
			.attr("text-anchor","middle")
...
/* Data */
	...
	/* Plotting Link */
			visChapter.append("line")
				.attr("x1", (lmargin))
			    .attr("y1", function(d) { return chapterScale(d.getElementsByTagName("order")[0].textContent) })
			    .attr("x2", function(d) {
			    	if (d.getElementsByTagName("date")[0].textContent == "Date Unknown") { return width / 2 }
			    	else { return (width - (rmargin)) }
		...
		/* Chapter */
			visChapter.append("circle")
					.attr("r",6)
					.attr("cy", function(d) { return chapterScale(d.getElementsByTagName("order")[0].textContent) })
					.attr("cx", (lmargin))
				.append("title")
					.text(function(d) {
					if (d.getElementsByTagName("date")[0].textContent == "Date Unknown") { return "Ch. "+d.getElementsByTagName("order")[0].textContent+". "+d.getElementsByTagName("title")[0].textContent+" (Date unknown.)" }
				...
				.append("title")
					...
					if (d.getElementsByTagName("date")[0].textContent == "Date Unknown") { return "Ch. "+d.getElementsByTagName("order")[0].textContent+". "+d.getElementsByTagName("title")[0].textContent+" (Date unknown.)" }
						...
						var sentDate = new Date(d.getElementsByTagName("date")[0]),
							...
							sentDay = sentDate.getDay();
						return "["+readFormat(new Date(1770,sentMonth,sentDay))+"] "+d.getElementsByTagName("order")[0].textContent+". "+d.getElementsByTagName("title")[0].textContent+" (From "+d.getElementsByTagName("name")[0].textContent+".)" }});
		$(domElement+" a.chapter *").tipsy({trigger:'focus',gravity:'e'});
		$(domElement+" a.chapter *").tipsy({gravity:'e'});
	});
}